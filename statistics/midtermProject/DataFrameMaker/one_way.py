        ...
        for idFriend in tDict[id]:
            if idFriend in tDict.keys():
                ...
                if(len(tDict[idFriend])>len(tDict[id])):
                    ...
                    FFNEFN = FFNEFN+1
        tFriend = tFriend.append( {'Id' : id, 'FNum': len(tDict[id]),'FFNum':idFriendFNum, 'FFNumAVG' : idFriendFNum/len(tDict[id]), "FFNLTFN" : FFNLTFN, "FFNGTFN" : FFNGTFN, "FFNEFN" : FFNEFN}, ignore_index = True)
    return tFriend
...
if __name__ == '__main__':
    ...
    result_dataframe.to_csv('./facebook_one_way.csv', sep='\t', na_rep='NaN', index=False)